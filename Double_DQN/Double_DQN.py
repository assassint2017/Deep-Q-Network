...
构建 Double DQN
和最原始的DQN相比，区别在于q_target的计算，
DDQN的q_target的值，是通过使用evel_net中的最大q值来为target_net选择动作计算出来的，
而不是直接选择target_net中最大的q值
"""
...
class DQN(object):
    ...
    def update_parameter(self):
        ...
        # 这部分就是ddqn相对于原始dqn的改动之处
        with torch.no_grad():

            action_index = eval_net(next_observation_batch).max(1)[1].unsqueeze(dim=1)
            q_next = target_net(next_observation_batch).gather(1, action_index)

            q_target = reward_batch + gamma * q_next * done_batch
